...
수정했음 이거추가했으니 초록줄 나와야함
수정
수정
수정
testBranch 에서 처음 commit 할꺼임!!!!!!
testBranch
testBranch
testBranch
secondCommit at testBranch!!!!!
branch는 달라졌는데 내컴퓨터에서 작업하는 위치랑 파일은 같은거임
그래서 항상 파일 올리기 전에 다른경로에 복사해 놓고 깃허브 올리세요

3nd commit at testBranch 수정사항 표기 되엉야 한다!!!!

main - testBranch 이런 관계일 때는
testBranch 에서 작업한 것을 main 에서 push 해야 수정결과가 보인다
단 push 할때는 최종 반영된 사항을 push 해야한다.
그러므로 branch 에서 수정사항을 계속 시도할때에는
branch 에서 branch 를 따서 push를 하는게 좋다.
...
--------------------------------------------------------
--  DDL for Table T_GOODS_DETAIL_IMAGE	--상품이미지정보
--------------------------------------------------------

  CREATE TABLE "T_GOODS_DETAIL_IMAGE" 
   ("IMAGE_ID" NUMBER(20,0) primary key,	--이미지번호
	"GOODS_ID" NUMBER(20,0),		--상품번호
	"FILENAME" VARCHAR2(50 BYTE),	--이미지파일명
	"REG_ID" VARCHAR2(20 BYTE),		--동록자아이디
	"FILETYPE" VARCHAR2(40 BYTE),	--이미지파일종류
...
--------------------------------------------------------
  ...
  CREATE TABLE "T_SHOPPING_GOODS" 
   ("GOODS_ID" NUMBER(20,0) primary key,	--상품번호
	--"GOODS_SORT" VARCHAR2(50 BYTE),		--상품종류
	"GOODS_TITLE" VARCHAR2(100 BYTE),		--상품제목
	--"GOODS_WRITER" VARCHAR2(50 BYTE),		--저자이름
	"GOODS_COUNTRY_OF_ORIGIN" VARCHAR2(50 BYTE),	--출판사이름 -> 원산지로 변경
	"GOODS_PRICE" NUMBER(10,0),				--상품정가
	"GOODS_SALES_PRICE" NUMBER(10,0),		--상품판매가격
	"GOODS_POINT" NUMBER(10,0),				--상품포인트
	--"GOODS_PUBLISHED_DATE" DATE,			--상품출판일
	--"GOODS_TOTAL_PAGE" NUMBER(5,0),		--상품총페이지수
	--"GOODS_ISBN" VARCHAR2(50 BYTE),		--isbn번호(국제 표준 도서 번호)
	"GOODS_DELIVERY_PRICE" NUMBER(10,0),	--배송비	
	"GOODS_DELIVERY_DATE" DATE,							--상품배송일--해당변수 어디에 사용되는지 확인 필요
	"GOODS_STATUS" VARCHAR2(50 BYTE),					--상품분류(베스트셀러, 스테디셀러 구분용도)
	--"GOODS_INTRO" VARCHAR2(2000 BYTE),				--저자소개
	--"GOODS_WRITER_INTRO" VARCHAR2(2000 BYTE),			--상품소개(p.1211 참고, 상품관리 기능에서 '상품이름'목록에서 해당 책 선택하면 보여지는 탭중 하나 )
	--"GOODS_PUBLISHER_COMMENT" VARCHAR2(2000 BYTE),	--출판사평(p.1211 참고)
	--"GOODS_RECOMMENDATION" VARCHAR2(2000 BYTE),		--상품추천사(p.1211 참고)
	--"GOODS_CONTENTS_ORDER" CLOB,						--상품목차(p.1211 참고)
	"GOODS_CREDATE" DATE DEFAULT sysdate				--상품입고일(p.1211 참고, 상품상세보기)

	--추가사항--
	"GOODS_DELIVERY_EXTRA_CHARGE" VARCHAR2(100 BYTE),	--산간배송비
	"GOODS_DELIVERY_FREE_CONDITION" VARCHAR2(100 BYTE),	--조건부무료배송
...
--------------------------------------------------------
  ...
  CREATE TABLE "T_SHOPPING_MEMBER" 
   ("MEMBER_ID" VARCHAR2(20 BYTE) primary key,	--회원아이디
	...
	--"MEMBER_GENDER" VARCHAR2(10 BYTE),	--성별
	"TEL1" VARCHAR2(20 BYTE),	--유선전화번호1
	"TEL2" VARCHAR2(20 BYTE),	--유선전화번호2
	"TEL3" VARCHAR2(20 BYTE),	--유선전화번호3
	--"HP1" VARCHAR2(20 BYTE),	--휴대폰번호1
	--"HP2" VARCHAR2(20 BYTE),	--휴대폰번호2
	--"HP3" VARCHAR2(20 BYTE),	--휴대폰번호3
	--"SMSSTS_YN" VARCHAR2(20 BYTE),	--SMS메시지수신여부
	"EMAIL1" VARCHAR2(20 BYTE),	--이메일주소1
	"EMAIL2" VARCHAR2(20 BYTE),	--이메일주소2
	--"EMAILSTS_YN" VARCHAR2(20 BYTE),	--이메일수신여부
	...
	--"DEL_YN" VARCHAR2(20 BYTE) DEFAULT 'N'	--회원달퇴유무
   ) ;
--------------------------------------------------------
...
--------------------------------------------------------
  ...
  CREATE TABLE "T_SHOPPING_ORDER" 
   ("ORDER_SEQ_NUM" NUMBER(20,0) primary key,	--주문상품일련번호
	"ORDER_ID" NUMBER(20,0),	--주문번호
	"MEMBER_ID" VARCHAR2(20 BYTE),	--주문자아이디(회원테이블의 회원아이디와 동일한 변수이름이므로 주의할 것)
	"GOODS_ID" NUMBER(20,0),	--상품번호(상품정보테이블의 '상품번호'와 동일하 변수이므로 주의할 것)
	"ORDERER_NAME" VARCHAR2(50 BYTE),	--주문자이름
	"GOODS_TITLE" VARCHAR2(100 BYTE),	--상품이름(상품정보테이블의 '상품제목'과 동일하 변수이므로 주의할 것)
	"ORDER_GOODS_QTY" NUMBER(5,0),	--주문수량
	"GOODS_SALES_PRICE" NUMBER(5,0),	--상품판매가격(상품정보테이블의 '상품판매가격'과 동일하 변수이므로 주의할 것)
	"GOODS_FILENAME" VARCHAR2(60 BYTE),	--상품이미지파일이름
	"RECEIVER_NAME" VARCHAR2(50 BYTE),	--수령자이름
	"RECEIVER_HP1" VARCHAR2(20 BYTE),	--수령자휴대폰번호1
	"RECEIVER_HP2" VARCHAR2(20 BYTE),	--수령자휴대폰번호2
	"RECEIVER_HP3" VARCHAR2(20 BYTE),	--수령자휴대폰번호3
	"RECEIVER_TEL1" VARCHAR2(20 BYTE),	--수령자유선전화번호1 
	"RECEIVER_TEL2" VARCHAR2(20 BYTE),	--수령자유선전화번호2 
	"RECEIVER_TEL3" VARCHAR2(20 BYTE),	--수령자유선전화번호3 
	"DELIVERY_ADDRESS" VARCHAR2(500 BYTE),	--배송주소 
	"DELIVERY_METHOD" VARCHAR2(40 BYTE),	--배송방법
	"DELIVERY_MESSAGE" VARCHAR2(300 BYTE),	--부재 시 전달 메시지
	"GIFT_WRAPPING" VARCHAR2(20 BYTE),	--상품포장여부
	"PAY_METHOD" VARCHAR2(200 BYTE),	--결제방법
	"CARD_COM_NAME" VARCHAR2(50 BYTE),	--결제카드회사이름
	"CARD_PAY_MONTH" VARCHAR2(20 BYTE),	--할부개월수
	"PAY_ORDERER_HP_NUM" VARCHAR2(20 BYTE),	--주문자 휴대폰번호
	"DELIVERY_STATE" VARCHAR2(20 BYTE) DEFAULT 'delivery_prepared',	--상품배송형태
	"PAY_ORDER_TIME" DATE DEFAULT sysdate,	--상품배송상태
	"ORDERER_HP" VARCHAR2(50 BYTE)	--결제시간
   ) ;
--------------------------------------------------------
